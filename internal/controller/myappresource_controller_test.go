...
var _ = Describe("MyAppResource controller", func() {
	Context("MyAppResource controller test", func() {
		...
		BeforeEach(func() {
			...
			err = os.Setenv("MYAPPRESOURCE_IMAGE", "example.com/image:test")
			Expect(err).To(Not(HaveOccurred()))
		...
		AfterEach(func() {
			By("removing the custom resource for the Kind MyAppResource")
			...
			_ = os.Unsetenv("MYAPPRESOURCE_IMAGE")
		})
		...
		It("should successfully reconcile a custom resource for MyAppResource", func() {
			...
			By("Checking the latest Status Condition added to the MyAppResource instance")
			Eventually(func() error {
				if myappresource.Status.Conditions != nil &&
					len(myappresource.Status.Conditions) != 0 {
					...
					expectedLatestStatusCondition := metav1.Condition{
						...
						Message: fmt.Sprintf(
							...
							myappresource.Name,
							myappresource.Spec.ReplicaCount),
					}
