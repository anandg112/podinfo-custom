...
import (
	...
	myv1alpha1 "github.com/anandg112/podinfo-custom.git/api/v1alpha1"
)

const myappresourceFinalizer = "my.api.group/finalizer"
...
func (r *MyAppResourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// The purpose is check if the Custom Resource for the Kind MyAppResource
	...
	if myappresource.Status.Conditions == nil || len(myappresource.Status.Conditions) == 0 {
		meta.SetStatusCondition(&myappresource.Status.Conditions, metav1.Condition{Type: typeAvailableMyAppResource, Status: metav1.ConditionUnknown, Reason: "Reconciling", Message: "Starting reconciliation"})
		if err = r.Status().Update(ctx, myappresource); err != nil {
			log.Error(err, "Failed to update MyAppResource status")
			return ctrl.Result{}, err
		}

		// Let's re-fetch the myappresource Custom Resource after update the status
	...
	// Check if the MyAppResource instance is marked to be deleted, which is
	...
	if isMyAppResourceMarkedToBeDeleted {
		if controllerutil.ContainsFinalizer(myappresource, myappresourceFinalizer) {
			...
			// Re-fetch the myappresource Custom Resource before update the status
			// so that we have the latest state of the resource on the cluster and we will avoid
			// raise the issue "the object has been modified, please apply
			// your changes to the latest version and try again" which would re-trigger the reconciliation
			if err := r.Get(ctx, req.NamespacedName, myappresource); err != nil {
			...
			log.Info("Removing Finalizer for MyAppResource after successfully perform the operations")
	...
	// The CRD API is defining that the MyAppResource type, have a MyAppResourceSpec.Size field
	// to set the quantity of Deployment instances is the desired state on the cluster.
	// Therefore, the following code will ensure the Deployment size is the same as defined
	// via the Size spec of the Custom Resource which we are reconciling.
	size := myappresource.Spec.ReplicaCount
	...
	meta.SetStatusCondition(&myappresource.Status.Conditions, metav1.Condition{Type: typeAvailableMyAppResource,
		...
		Message: fmt.Sprintf("Deployment for custom resource (%s) with %d replicas created successfully", myappresource.Name, size)})
...
// finalizeMyAppResource will perform the required operations before delete the CR.
...
func (r *MyAppResourceReconciler) deploymentForMyAppResource(
	myappresource *myv1alpha1.MyAppResource) (*appsv1.Deployment, error) {
				...
				Spec: corev1.PodSpec{
					...
					Containers: []corev1.Container{{
						...
						Ports: []corev1.ContainerPort{{
							ContainerPort: myappresource.Spec.ContainerPort,
							Name:          "myappresource",
						}},
						Command: []string{"./podinfo", "--port=9898", "--cache-server=cache:6379", "--level=info"},
					}},
				},
			},
		},
...
// from the MYAPPRESOURCE_IMAGE environment variable defined in the config/manager/manager.yaml
func imageForMyAppResource() (string, error) {
	var imageEnvVar = "MYAPPRESOURCE_IMAGE"
